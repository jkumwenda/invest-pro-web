  ...
  <div>
    ...
    <div class="my-5 font-semibold">
      <form
        ...
        v-on:submit.prevent="updateAnalysisPortifolio"
      >
        ...
        <div class="w-4/4 md:w-3/4 mb-6 mr-auto">
          <input
            ...
            name="analysis_date"
            v-model="analysisPortifolioData.analysis_date"
          />
        </div>
  ...
  methods: {
    getAnalysisPortifolio() {
      get(this.$axios, "analysis_portfolio/" + this.$route.params.id + "/")
        .then((result) => {
          this.analysisPortifolioData.company_id = result.company_id;
          this.analysisPortifolioData.analysis_date = result.analysis_date;
        })
        .catch((error) => {
          console.log(error);
        });
    ...
    updateAnalysisPortifolio() {
      update(
        this.$axios,
        "analysis_portfolio/" + this.$route.params.id + "/",
        this.analysisPortifolioData
      )
        .then((results) => {
