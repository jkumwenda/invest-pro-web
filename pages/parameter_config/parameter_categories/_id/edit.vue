<template>
  <div>
    ...
    >
      <h2 class="flex-1 text-xl font-semibold">Update parameter category</h2>
      <div></div>
    </div>
    <div class="my-5 font-semibold">
      <form
        class="w-8/12 bg-white rounded-xl p-8"
        v-on:submit.prevent="updateParameterCategory"
      >
...
export default Vue.extend({
  ...
  methods: {
    getParameterCategory() {
      get(this.$axios, "parameter_category/" + this.$route.params.id + "/")
        .then((result) => {
          this.parameterCategoryData.parameter_category =
            result.parameter_category;
        })
        .catch((error) => {
          console.log(error);
        });
    ...
    updateParameterCategory() {
      update(
        this.$axios,
        "parameter_category/" + this.$route.params.id + "/",
        this.parameterCategoryData
      )
        .then((results) => {
